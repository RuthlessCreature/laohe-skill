---
name: ~wiki
description: 初始化或同步项目知识库（仅 `.qualityagents/`）
policy:
...
执行 `~wiki` 时，`.qualityagents/` 目录结构、模板格式和状态文件重写规则按当前已加载的 QualityAGENTS 规则执行；不写入项目级规则文件，也不创建项目级 QualityAGENTS 包根链接。
`.qualityagents/` 在本 skill 中统一按项目级存储路径理解：状态文件只使用 `state_path`；若 `project_store_mode=repo-shared`，`context.md`、`guidelines.md`、`verify.yaml`、`CHANGELOG.md`、`DESIGN.md`、`modules/` 改按当前上下文中已注入的项目知识目录写入。

## 流程

### 阶段 1：基础准备（必做）

1. 创建 `.qualityagents/` 目录 + `state_path`（按 templates/STATE.md 格式）；初始“主线目标”只写当前知识库初始化 / 同步目标，不把它写成长期项目总目标
...
3. 明确不执行以下操作：
   ...
   - 不创建项目级 QualityAGENTS 包根链接

### 阶段 2：知识库创建或补全（条件性）

检查项目是否有实际代码文件（非空项目）：
- 有代码文件 → 执行完整知识库创建/补全（下方流程）
- 空项目 → 保留 `.qualityagents/` 和 `state_path`，告知用户“项目为空，其余知识文件将在后续开发或首次编码任务中补全”

知识库创建/补全流程（统一写入 `.qualityagents/` 对应的项目级存储路径；`project_store_mode=repo-shared` 时实际落在共享知识目录）：
